...
export const useAuthUpdate = (userId) => {
    const { getAccessTokenSilently } = useAuth0();
    const [authUserInfo, setAuthUserInfo] = useState();
    const [authLoading, setAuthLoading] = useState(true);


    const actualizarAuthUserInfo = async () => {
        try {
            const accessToken = await getAccessTokenSilently();
            if (userId) {
                const response = await getAuthUser(accessToken, userId);
    ...
    useEffect(() => {
        actualizarAuthUserInfo().then(() => setAuthLoading(false));
    }, [userId]);
...
export const useAuthAsignRol = (userId) => {
    const { getAccessTokenSilently } = useAuth0();
    const [authUserInfo, setAuthUserInfo] = useState();
    const [authLoading, setAuthLoading] = useState(true);
